		...
		<script>
			$(document).ready(function() {
				...
				$(add_question).click(function(e){ //on add input button click
					e.preventDefault();
						$("div#Questions").append("<div class='poll_wrap' id=" + (x) + "><input type='text' class= 'questionbox' name='myquestions[]'/><a href='#' class='remove_field'>Remove</a> <div id= group"+ (x) +" class='choices_wrap' value=0><a id= button"+x+ " class='add_choice'>Add Choice</a></div></div>"); //add input box
						...
						choice.addEventListener( "click", function( event ) {
							...
							$(choicewrapper).append("<input type='text' class='choicebox' name='mychoices[]'/>");
						},  false );
					x++;
				});
